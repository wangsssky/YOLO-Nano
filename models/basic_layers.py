...
class YOLOLayer(nn.Module):
    ...
    def forward(self, x, targets=None, img_dim=None):
        ...
        if targets is None:
            ...
            # loss_conf_obj = self.bce_logits_loss(prediction[..., 4][obj_mask], tconf[obj_mask])
            loss_conf_noobj = self.bce_loss(pred_conf[noobj_mask], tconf[noobj_mask])
            # loss_conf_noobj = self.bce_logits_loss(prediction[..., 4][noobj_mask], tconf[noobj_mask])
            loss_conf = self.obj_scale * loss_conf_obj + self.noobj_scale * loss_conf_noobj
            loss_cls = self.bce_loss(pred_cls[obj_mask], tcls[obj_mask])
            # loss_cls = self.bce_logits_loss(prediction[..., 5:][obj_mask],tcls[obj_mask])
